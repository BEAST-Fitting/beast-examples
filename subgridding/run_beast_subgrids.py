...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--merge", action="store_true", help="Merge the subgrid results"
    )
    parser.add_argument(
        "--ignore-missing-subresults",
        action="store_true",
    ...
    if args.physicsmodel:
        ...
        required_names = [
            "{}seds.gridsub{}.hd5".format(file_prefix, i) for i in range(args.nsubs)
        ]
    ...
    if args.ast:
        ...
        if len(mag_cuts) == 1:
            ...
            mag_cuts = (
                faintest_mags + tmp_cuts
            )  # this many mags fainter thant the 90th percentile
            bright_cuts = (
                brightest_mags - tmp_cuts
    ...
    if args.fit:
        ...
        def fit_submodel(modelsedgridfile):
            # input files
            trimmed_modelsedgridfile = modelsedgridfile.replace("seds", "seds_trim")
            ...
            noisemodel_vals = noisemodel.get_noisemodelcat(trimmed_noisemodelfile)
